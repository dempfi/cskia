...
void sk_paragraph_get_glyph_position_at_coordinate(sk_paragraph_t* self, float dx, float dy, sk_positionaffinity_t* result) {
...
size_t sk_paragraph_get_line_metrics(sk_paragraph_t* self, sk_metrics_t result[]) {
  if (result) {
...
size_t sk_paragraph_get_rects_for_placeholders(sk_paragraph_t* self, sk_textbox_t result[]) {
...
size_t sk_paragraph_get_rects_for_range(sk_paragraph_t* self, uint32_t start, uint32_t end, sk_rectheightstyle_t rect_height_style, sk_rectwidthstyle_t rect_width_style, sk_textbox_t result[]) {
...
size_t sk_paragraph_get_line_number(sk_paragraph_t* self) {
  return AsParagraph(self)->lineNumber();
};
...
void sk_paragraph_visit(sk_paragraph_t* self, sk_paragraph_visit_proc proc, void* proc_context) {
  AsParagraph(self)->visit([&](int lineNumber, const skia::textlayout::Paragraph::VisitorInfo* info) { proc(lineNumber, ToParagraphVisitorInfo(info), proc_context); });
}
