...
sk_patheffect_t* sk_patheffect_make_matrix(const sk_matrix_t* matrix) {
  return ToPathEffect(SkMatrixPathEffect::Make(AsMatrix(matrix)).release());
}

sk_patheffect_t* sk_patheffect_make_merge(sk_patheffect_t* effect1, sk_patheffect_t* effect2, sk_pathop_t op) {
...
sk_patheffect_t* sk_patheffect_make_stroke(float width, sk_strokejoin_t join, sk_strokecap_t cap, float miter) {
  return ToPathEffect(SkStrokePathEffect::Make(width, AsStrokeJoin(join), AsStrokeCap(cap), miter).release());
}

sk_patheffect_t* sk_patheffect_make_stroke_and_fill(void) {
...
sk_patheffect_t* sk_patheffect_make_translate(float dx, float dy) {
