...
void sk_codec_get_dimensions(const sk_codec_t* self, sk_isize_t* result) {
  *result = ToISize(AsCodec(self)->dimensions());
}

sk_encodedimageformat_t sk_codec_get_encoded_image_format(const sk_codec_t* self) {
  return ToEncodedImageFormat(AsCodec(self)->getEncodedFormat());
}

sk_image_t* sk_codec_get_image(sk_codec_t* self, sk_colortype_t color_type, sk_alphatype_t alpha_type, sk_colorspace_t* color_space) {
...
bool sk_codec_get_pixels(sk_codec_t* self, void* pixels, size_t row_bytes, sk_colortype_t color_type, sk_alphatype_t alpha_type, sk_colorspace_t* color_space) {
...
sk_codec_t* sk_codec_make_from_file(const char file_name[]) {
  auto s = std::make_unique<SkFILEStream>(file_name);
  return ToCodec(SkCodec::MakeFromStream(std::move(s)).release());
}

sk_codec_t* sk_codec_make_from_stream(sk_stream_t* stream) {
  std::unique_ptr<SkStream> s(AsStream(stream));
  return ToCodec(SkCodec::MakeFromStream(std::move(s)).release());
}

sk_codec_t* sk_codec_make_with_copy(const void* data, size_t size) {
  return ToCodec(SkCodec::MakeFromData(SkData::MakeWithCopy(data, size)).release());
}

sk_codec_t* sk_codec_make_without_copy(const void* data, size_t size) {
  return ToCodec(SkCodec::MakeFromData(SkData::MakeWithoutCopy(data, size)).release());
}

void sk_animcodecplayer_destroy(sk_animcodecplayer_t* self) {
  delete AsAnimCodecPlayer(self);
}

void sk_animcodecplayer_get_dimensions(const sk_animcodecplayer_t* self, sk_isize_t* result) {
  *result = ToISize(AsAnimCodecPlayer(self)->dimensions());
}

uint32_t sk_animcodecplayer_get_duration(const sk_animcodecplayer_t* self) {
  return AsAnimCodecPlayer(self)->duration();
}

sk_image_t* sk_animcodecplayer_get_frame(sk_animcodecplayer_t* self) {
  return ToImage(AsAnimCodecPlayer(self)->getFrame().release());
}

sk_animcodecplayer_t* sk_animcodecplayer_make_from_file(const char file_name[]) {
  auto s = std::make_unique<SkFILEStream>(file_name);
...
sk_animcodecplayer_t* sk_animcodecplayer_make_from_stream(sk_stream_t* stream) {
  std::unique_ptr<SkStream> s(AsStream(stream));
  if (auto codec = SkCodec::MakeFromStream(std::move(s))) {
    return ToAnimCodecPlayer(new SkAnimCodecPlayer(std::move(codec)));
  }
...
bool sk_animcodecplayer_seek(sk_animcodecplayer_t* self, uint32_t milliseconds) {
