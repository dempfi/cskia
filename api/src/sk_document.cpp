...
sk_canvas_t *sk_document_begin_page(sk_document_t *self, float width, float height, const sk_rect_t *content)
{
  return ToCanvas(AsDocument(self)->beginPage(width, height, AsRect(content)));
}

void sk_document_close(sk_document_t *self)
{
  AsDocument(self)->close();
}

void sk_document_end_page(sk_document_t *self)
{
  AsDocument(self)->endPage();
}

sk_document_t *sk_document_make_pdf(sk_wstream_t *w_stream)
{
  return ToDocument(SkPDF::MakeDocument(AsWStream(w_stream)).release());
}

sk_document_t *sk_document_make_pdf2(sk_wstream_t *w_stream, const sk_pdfmetadata_t *metadata)
{
  return ToDocument(SkPDF::MakeDocument(AsWStream(w_stream), AsPDFMetadata(metadata)).release());
}

sk_document_t *sk_document_make_xps(sk_wstream_t *w_stream, float dpi)
{
  SK_ONLY_XPS(
      IXpsOMObjectFactory *factory = get_xps_factory();
      if (!factory) return nullptr;)
  return SK_ONLY_XPS(ToDocument(SkXPS::MakeDocument(AsWStream(w_stream), factory, dpi).release()), nullptr);
}

void sk_document_terminate(sk_document_t *self)
{
