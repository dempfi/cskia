...
public final class Textblob {
  ...
  init? (str: String, font: Font, origin: SkPoint = SkPoint(x: 0, y: 0)) {
    ...
    sk_font_get_glyphs(font.handle, str, str.utf8.count, UTF8_SK_TEXTENCODING, UnsafeMutablePointer<UInt16>(OpaquePointer(tbb.glyphs!)), nglyphs)
    var o = origin

    sk_font_get_positions(font.handle, UnsafeMutablePointer<UInt16>(OpaquePointer(tbb.glyphs!)), nglyphs, UnsafeMutablePointer<sk_point_t>(OpaquePointer(tbb.pos!)), &o)
...
public final class TextBlobBuilder {
  ...
  /// - Parameter y: The vertical offset within the blob.
  /// - Parameter glyphs: The glyphs to use for this run
  public func addPositionedRun(font: Font, y: Float, glyphs: [ushort], positions: [SkPoint]) {
    let run = allocateHorizontalRun(font: font, count: Int32(glyphs.count), y: y)
    run.clusters.copyMemory(from: glyphs, byteCount: glyphs.count * 2)
    run.pos.copyMemory(from: positions, byteCount: positions.count * MemoryLayout<SkPoint>.size)
  }
   ...
   */
  func allocateRun(font: Font, count: Int32, x: Float, y: Float, textByteCount: Int32 = 0, bounds: SkRect? = nil) -> sk_textblob_builder_runbuffer_t {
  ...
  func allocateHorizontalRun(font: Font, count: Int32, y: Float, textByteCount: Int32 = 0, bounds: SkRect? = nil) -> sk_textblob_builder_runbuffer_t {
    var ret = sk_textblob_builder_runbuffer_t()
    if let b = bounds {
      var nb = b
  ...
  func allocatePositionedRun(font: Font, count: Int32, textByteCount: Int32 = 0, bounds: SkRect? = nil) -> sk_textblob_builder_runbuffer_t {
    var ret = sk_textblob_builder_runbuffer_t()
    if let b = bounds {
      var nb = b
