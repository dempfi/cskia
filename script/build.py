...
def embed_api():
  ...
  include_src = Path("../api/include")
  include_dst = Path("../skia/include")
  src_src = Path("../api/src")
  src_dst = Path("../skia/src")
  gni_file = Path("gn/core.gni")
  ...
  # Find the line with 'skia_core_sources = ['
  start_index = next((i for i, line in enumerate(lines) if line.strip().startswith("skia_core_sources = [")), None)

  if start_index is None:
    raise ValueError("Could not find 'skia_core_sources = [' in the gni file")

  # Get the filenames from the src directory
  src_files = [f.name for f in src_src.glob("*")]

  # Check if there are any files in the gni file, stop if they are
  for line in lines[start_index+1:]:
    if any(file in line for file in src_files):
      print("Some files from ../api/src already exist in the gni file")
      return

  # Otherwise, insert the filenames
  for file in src_files:
    lines.insert(start_index+1, f"  \"$_src/{file}\",\n")
